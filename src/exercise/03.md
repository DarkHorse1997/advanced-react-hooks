...
1. useContext should be used only in some specific cases, like when we need the
   same value in a lot of components, such as theme. It is not always the
   solution to prop-drilling. See the link below for more about that.

2. You can create Context Provider and Context Consumer components separately
   for better DX.
